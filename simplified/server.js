...
import {
  createSimplifiedSession,
  finalizeSimplifiedSession
} from '../src/simplified-storage.js';
...
export function createSimplifiedAppServer({ publicDir = PUBLIC_DIR, dataRoot = DATA_ROOT, sharedDir = SHARED_DIR } = {}) {
  return http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/health') {
        return json(res, { ok: true, workflow: 'simplified' });
      }
      if (req.method === 'POST' && url.pathname === '/api/session') {
        ...
        const session = await createSimplifiedSession(dataRoot, {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  createSimplifiedAppServer().listen(PORT, () => {
    console.log(`Simplified rPPG x Polar H10 validation app: http://localhost:${PORT}`);
