...
4. The app runs the HCT protocol:
   ...
   - 30-second active cool-down at 80 SPM.
   - 10-second pre-recovery rPPG buffering.
   - Four 30-second recovery trials.
...
- `participants_summary.csv`: one row per participant/session, including phase-level CWT/LS agreement metrics and serialized trial metrics.

## Trial-Level Methodology

For each formal baseline and recovery trial, the analysis uses the `trial_start` and `trial_end` event markers as the authoritative counting window. Participant submission time is recorded as a separate event and does not extend the physiological trial window.
...
CWT and LS heartbeat counts are calculated by the rPPG pipeline:
...
For CWT and LS, the app reports:
...
These metrics are stored separately for:

- Baseline CWT vs Polar.
- Baseline LS vs Polar.
- Recovery CWT vs Polar.
- Recovery LS vs Polar.
...
- Trial-level and phase-level validation metrics are implemented and covered by tests.
